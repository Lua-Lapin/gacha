...
| プレースホルダ | `{役職名}` / `{カクテル名}` は原文のまま。「置き換えて使ってください」の注意書きを添える |
...
- 最上部に注意書きを常時表示する: 「`{役職名}` `{カクテル名}` の部分は、自分の役職名に置き換えて使ってください」。カクテルガチャのテンプレートだけ `{カクテル名}` を使うため、注意書きは両方に触れる固定文言とする（プレースホルダ名の動的検出はしない）
...
- `promptsFor` — 複数スタイルのガチャ（sea のみ: card / jacket）、1 スタイルのガチャ（cocktail, izakaya, sushi）、未知IDで空配列
- `renderPrompts` — 本文が全文出ること、`{役職名}` および `{カクテル名}` が原文のまま残ること、注意書きが出ること、HTML特殊文字がエスケープされること、スタイルが 1 つならサブタブが出ないこと
